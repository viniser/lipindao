...
//        $sign = $this->request->request("sign");
        ...
        $md5key="8516123e";
        $getkey=$this->request->request('key');//接收参数key
        $tradeNo=$this->request->request('tno');//接收参数tno 交易号
        $desc=$this->request->request('payno');//接收参数payno 一般是用户名 用户ID
        $price=$this->request->request('money');//接收参数money 付款金额
        $sign=$this->request->request('sign');//接收参数sign
        $type = 'alipay';

        //验证签名
      //  if ($sign != md5(md5($price . $type) . $this->config['secretkey'])) {
        if(strtoupper($sign) != strtoupper(md5($tradeNo.$desc.$price.$md5key))){
            $this->error("签名不正确");
        }
